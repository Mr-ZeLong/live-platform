...
import com.logilong.live.gift.bo.RollBackStockBO;
import com.logilong.live.gift.constants.SkuOrderInfoEnum;
import com.logilong.live.gift.dto.*;
import com.logilong.live.gift.interfaces.ISkuOrderInfoRpc;
import com.logilong.live.gift.provider.dao.po.SkuInfoPO;
import com.logilong.live.gift.provider.dao.po.SkuOrderInfoPO;
import com.logilong.live.gift.provider.service.IShopCarService;
import com.logilong.live.gift.provider.service.ISkuInfoService;
import com.logilong.live.gift.provider.service.ISkuOrderInfoService;
import com.logilong.live.gift.provider.service.ISkuStockInfoService;
...
public class SkuOrderInfoRpcImpl implements ISkuOrderInfoRpc {
    ...
    public SkuPrepareOrderInfoDTO prepareOrder(PrepareOrderReqDTO reqDTO) {
        ...
        try {
            ...
            LOGGER.info("[prepareOrder] sendResult is {}", sendResult);
        } catch (Exception e) {
            LOGGER.error("[prepareOrder] error is: ", e);
        }
    ...
    public boolean payNow(Long userId, Integer roomId) {
        ...
        // 获取余额并判断余额是否充足
        Integer balance = liveCurrencyAccountRpc.getBalance(userId);
        if (balance < totalPrice) {
            return false;
        }
        // 余额扣减
        liveCurrencyAccountRpc.decr(userId, totalPrice);
        // 更改订单状态未已支付
